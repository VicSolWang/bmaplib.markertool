 ...
 * @example <b>参考示例：</b><br />
 */
const MarkerTool = (window.BMapLib.MarkerTool = function (map, opts) {
	try {
		BMap;
	} catch (e) {
 ...
 * @param {Object} 参数
 */
MarkerTool.prototype._initialize = function (map, opts) {
	/**
	 * map对象
	 * @private
	 * @type {Map}
	...
	MarkerTool.SYS_ICONS = [
		...
		new BMap.Icon(MarkerTool.ICON_IMG, new BMap.Size(23, 25), {
		...
		new BMap.Icon(MarkerTool.ICON_IMG, new BMap.Size(25, 25), {
		...
		new BMap.Icon(MarkerTool.ICON_IMG, new BMap.Size(24, 25), {
			anchor: new BMap.Size(12, 25),
		...
		new BMap.Icon(MarkerTool.ICON_IMG, new BMap.Size(19, 25), {
			anchor: new BMap.Size(9, 25),
	...
	this._opts.icon = this._opts.icon || MarkerTool.SYS_ICONS[8]; // 默认选择红色雨滴样式
...
MarkerTool.prototype._bind = function () {
	...
	me._clickHandler = function (evt) {
		const evtPix = evt.pixel;
		// 补偿_followMarker的-10像素问题,解决cursor问题
		const iconPix = new BMap.Pixel(evtPix.x - 10, evtPix.y - 10);
		const pt = me._map.pixelToPoint(iconPix);
		const mkr = new BMap.Marker(pt, { icon: me._opts.icon });
		 ...
		 */
		const event = new baidu.lang.Event('onmarkend');
		event.marker = mkr;
